...
Launch files and configuration for alfa_robot.
...
1. If you want to test hardware with `ForwardCommandController` first load and configure it. Controller types are e.g., "position", "velocity", and depend on configured names in the [`config/alfa_robot_controllers.yaml`](config/alfa_robot_controllers.yaml):
   ```
