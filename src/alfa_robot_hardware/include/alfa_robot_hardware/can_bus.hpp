...
public:
  ...
  struct TrajectoryLogEntry
  {
    double time_s{0.0};
    uint8_t motor_id{0};
    double p_cmd{0.0};    // Position command sent to motor
    double v_cmd{0.0};    // Reserved for future diagnostics
    double a_cmd{0.0};    // Reserved for future diagnostics
    double p_raw{0.0};    // Raw position command before filtering
  };
