...
int main(int argc, char** argv)
{
    ...
    // 2. 指定目标 Pose 的参考坐标系。
    // 这会影响 setPoseTarget() 传入位姿的解释方式，
    // 但不会改变 MoveIt 机器人模型的 planning frame。
    std::string reference_frame = "base_link"; 
    move_group.setPoseReferenceFrame(reference_frame);

    // 3. (可选) 强制指定末端执行器连杆
    // 如果你知道 RViz 里拖拽的是哪个连杆，在这里显式指定它
     std::string eef_link = "right_ee_link";
    ...
    target_pose_stamped.header.stamp = node->now();
    target_pose_stamped.pose = target_pose;

    // 输出基准信息供排查。
    // planning frame 通常由 SRDF/虚拟关节决定；pose reference frame 才是 setPoseTarget() 使用的参考系。
    RCLCPP_INFO(node->get_logger(), "【检查对齐】Planning frame: %s", move_group.getPlanningFrame().c_str());
    RCLCPP_INFO(node->get_logger(), "【检查对齐】Pose reference frame: %s", move_group.getPoseReferenceFrame().c_str());
    RCLCPP_INFO(node->get_logger(), "【检查对齐】末端执行器连杆: %s", move_group.getEndEffectorLink().c_str());
    RCLCPP_INFO(
        node->get_logger(),
        "【检查对齐】发送给 MoveIt 的目标位姿: frame=%s, xyz=[%.3f, %.3f, %.3f]",
    ...
    // ================= 3. 规划与执行 =================
    move_group.setPoseTarget(target_pose_stamped, eef_link);
