...
- 命名规范：Linear 标题以后优先使用“任务命名/主题前缀”而不是把编号放最前；编号可放标题末尾备注。例如 `IK 选优探索 / Updown 查表：固定高度后双臂 BioIK 联合求解（T-0006A）`。
- 标题已同步：`TIM-31` 改为 `IK 选优探索：...（T-0006）`，`TIM-34/35/36` 改为 `IK 选优探索 / Updown 查表|代价注入|候选评分：...（T-0006A/B/C）`。
- 追加同步：`TIM-32/T-0007` 改为 `IK 选优探索 / 安全距离：双臂最小距离查询与近碰撞惩罚（T-0007）`；`TIM-33/T-0008` 改为 `IK 选优探索 / Rerun 对比：显示候选解、失败姿态和代价分解（T-0008）`。
