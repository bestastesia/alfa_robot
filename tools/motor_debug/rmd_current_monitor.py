...
用法:
  python3 rmd_current_monitor.py --can can1 --id 4,5,6
  python3 rmd_current_monitor.py --can can0 --id 1
  python3 rmd_current_monitor.py --can can0 --id 1 --interval 20 --window 30
...
class RealtimePlotter:
    def __init__(self, collector: MotorDataCollector, window_sec: float = 10.0):
        ...
        self.fig.canvas.manager.set_window_title("RMD Motor Current Monitor")
        ...
        self.title = self.fig.suptitle(
            f"RMD 转矩电流监控 | CAN: {collector.can_interface} | 电机: {ids_str} | 运行中",
    ...
    def _on_pause(self, event=None):
        ...
        self.title.set_text(
            f"RMD 转矩电流监控 | CAN: {self.collector.can_interface} | 电机: {ids_str} | {state}"
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="RMD 电机转矩电流实时波形监控",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
示例:
  %(prog)s --can can0 --id 1
  %(prog)s --can can0 --id 1,2,3
  %(prog)s --can can0 --id 1 --window 20 --interval 30
""",
    )
    parser.add_argument("--can", default="can1", help="CAN 接口 (默认: can1)")
    parser.add_argument("--id", default="6", help="电机ID, 如 1,2,3 或 1-6")
    parser.add_argument("--signals", default="current",
                        help="信号: speed,position,current,temperature (默认: current)")
