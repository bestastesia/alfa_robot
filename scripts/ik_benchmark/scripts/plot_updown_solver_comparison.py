        ...
        rows.append(
            {
                ...
                "合法候选数": int(rec.get("legal_count", 0) or 0),
...
def write_markdown(summary: pd.DataFrame, df: pd.DataFrame, output: Path, source: Path) -> None:
...
def main() -> None:
    ...
    write_markdown(summary, df, output_dir / "实验汇总.md", args.jsonl)
    ...
    metadata = {
        ...
        "charts_note": "joint2/joint3 lever charts require joint lever fields in JSONL; if missing, chart files may contain no curves.",
    }
