...
md=['# 13组任务 Updown 全高度扫描分析','', '- 扫描申请范围：`0.00~0.99 m`，步长 `0.01 m`。','- 当前 URDF 逻辑范围与电机物理满行程均为：`0.00~0.70 m`（零偏移合同，physical=logical）；该范围外仅用于暴露配置越界，不纳入推荐。','- Joint2 限位：左右均为 `±90°`。','- “可用区间”：该高度至少存在1个附着箱体场景碰撞通过的双臂IK解。','- “稳健区间”：该高度的通过分支数不少于本任务单高度最大通过分支的50%。','', '|任务|可用区间(m)|稳健区间(m)|每高度最多通过分支|碰撞通过/检查|总通过率|主要碰撞原因|','|---|---|---|---:|---:|---:|---|']
for s in summaries:
