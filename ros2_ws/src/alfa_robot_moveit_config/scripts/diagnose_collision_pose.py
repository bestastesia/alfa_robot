...
用途:诊断 rightjoint1/2/3<->rightjoint6 自碰撞误报到底是
...
POSE = {
    ...
    "leftjoint1": 0.00038, "leftjoint2": -0.78748, "leftjoint3": -2.11524,
    "leftjoint4": -1.30916, "leftjoint5": -0.00018, "leftjoint6": 7e-05,
    "rightjoint1": -0.00042, "rightjoint2": 0.78708, "rightjoint3": 2.11644,
    "rightjoint4": 1.30811, "rightjoint5": -0.00026, "rightjoint6": 0.0023,
}
...
def main() -> int:
    ...
    # 数值分析:右臂各 link 原点两两距离,重点看 rightjoint6 与 rightjoint1/2/3
    print("=== 右臂 link 原点(base 系, m) ===")
    right_links = ["rightjoint1", "rightjoint2", "rightjoint3", "rightjoint4",
                   "rightjoint5", "rightjoint6", "right_tool0"]
    origins = {}
    ...
    print("\n=== rightjoint6 与 rightjoint1/2/3 原点间距(m) ===")
    j6 = origins.get("rightjoint6")
    for ln in ["rightjoint1", "rightjoint2", "rightjoint3"]:
        o = origins.get(ln)
        if j6 is not None and o is not None:
            print(f"  |rightjoint6 - {ln}| = {np.linalg.norm(j6 - o):.4f}")

    # 对称对照:左臂(镜像姿态)同样的间距,看是否一致(判断是否单纯折叠)
    print("\n=== 左臂对照 leftjoint6 与 leftjoint1/2/3 间距(m) ===")
    l6 = link_origin(fk, "leftjoint6")
    for ln in ["leftjoint1", "leftjoint2", "leftjoint3"]:
        o = link_origin(fk, ln)
        if l6 is not None and o is not None:
            print(f"  |leftjoint6 - {ln}| = {np.linalg.norm(l6 - o):.4f}")
    ...
    # 标出被误报的 link 原点
    for ln in ["rightjoint1", "rightjoint2", "rightjoint3", "rightjoint6"]:
        o = origins.get(ln)
        if o is not None:
