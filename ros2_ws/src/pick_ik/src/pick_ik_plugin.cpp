...
bool PickIKPlugin::searchPositionIK(std::vector<geometry_msgs::msg::Pose> const& ik_poses,
    ...
    // Create robot state for FK calculations
    auto robot_state = moveit::core::RobotState(robot_model_);
    robot_state.setToDefaultValues();
    robot_state.setJointGroupPositions(jmg_, ik_seed_state);
    robot_state.update();

    // Transform goal poses to frames
    auto goal_frames = transform_poses_to_frames(robot_state, ik_poses, getBaseFrame());

    // If fix_unspecified_end_effectors is enabled and we have more tip_frames than ik_poses,
    // add constraints for the unspecified end effectors to stay at their current pose
    std::vector<size_t> goal_tip_indices;  // which tip_indices have goals
    if (params.fix_unspecified_end_effectors && tip_link_indices_.size() > ik_poses.size()) {
        // Mark which tips have goals (assume ik_poses correspond to first N tips)
        for (size_t i = 0; i < ik_poses.size(); ++i) {
            goal_tip_indices.push_back(i);
        }

        // For remaining tips, add their current pose as goal (fix them in place)
        for (size_t i = ik_poses.size(); i < tip_link_indices_.size(); ++i) {
            auto const tip_index = tip_link_indices_[i];
            auto const& tip_frame = robot_state.getGlobalLinkTransform(
                robot_model_->getLinkModel(tip_index));
            goal_frames.push_back(tip_frame);
            RCLCPP_DEBUG(LOGGER, "Fixing end effector %zu at current pose", i);
        }
    }
