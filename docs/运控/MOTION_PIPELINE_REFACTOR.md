...
- RRT 现在同时保留原始树路径和 shortcut 路径，并定期从不同树节点尝试连接终点；候选按完整路径去重，不再把不同分支过早压缩成同一条直连。L2/R3、L7/R4 均能生成左右各 8 条路径，并实际检查 64 组双臂组合。
- L2/R3 的左右单臂隔离检查均为 `0/8`：左臂主要在第 8 个离散步发生 `leftjoint2 <-> updown`，右臂主要在第 7 个离散步发生 `front_sensor <-> rightjoint3`。L7/R4 同样是左右单臂分别撞 `joint2 <-> updown`。因此失败不是“双臂同步方式不佳”，而是当前模型在严格 `retreat + pitch` 二维空间内的单臂路径本身即不满足碰撞约束。
- 顶吸改用已知解析 IK 可达的 L22/R24 检查：抓取 IK 有 256 个合法解析解，但选出的 8 个候选在附着箱和统一场景过滤后全部被拒绝，原因为附着箱与下方箱墙重叠或 `joint2 <-> updown`。因此顶吸 RRT 尚未获得合法起点，不能通过修改搜索器掩盖该场景问题。
- `extract_box_pose_rrt_diagnostics` 默认关闭；仅在定位问题时启用单臂隔离碰撞统计，避免正式实验额外重复碰撞检查。
- 当前约束下没有得到真实碰撞合法的典型路径，所以稳定默认仍保持 `greedy`。如需继续提高成功率，必须先评审并明确增加搜索自由度或调整机械/场景约束，不能静默忽略雷达、立柱、附着箱或箱墙碰撞。
