...
**用途**：输入车头到箱墙的距离和箱号，先升降到肩心高于箱体中心 `0.25m`，再接近、吸取、抽出、携箱回 home、升降归零。包含仓库和邻箱碰撞检查。
...
**用途**：将第1版的固定 `0.25m` 偏移换成“肩心到吸取TCP的距离比例”选高；每臂只选一次高度，其余流程相同。默认候选比例 `[1.10,1.15]`、首选 `1.15`；**独立验证没有证明优于旧版**。
...
**用途**：在 RViz 拖动单个箱子的 XYZ，验证接近、吸取、抽出和返回；不是按箱号选择仓库箱墙。
