    ...
    const UpdownAwareIkRequest& request, const HeightPlan& plan) const
{
    ...
    const size_t h_count = std::max<size_t>(1, plan.candidates.size());
    const size_t target_trial_count = worker_count * seed_count;
    ...
    while (!plan.candidates.empty() && trial_count < target_trial_count) {
        for (size_t seed_index = 0; seed_index < seed_count && trial_count < target_trial_count; ++seed_index) {
            for (size_t worker_slot = 0; worker_slot < worker_count && trial_count < target_trial_count; ++worker_slot) {
                const size_t h_index = trial_count % h_count;
                TrialSpec trial;
                ...
                trial.seed_index = seed_index + worker_slot * seed_count + repeat_index * worker_count * seed_count;
                trial.solver_path = h_index == 0 && std::abs(trial.h - request.current_h) < 1e-9
                    ? "fixed_current_h"
                    : "fixed_h_candidates";
                trial.seed = trial.seed_index == 0
                    ? base_seed
                    : makePerturbedSeed(fixed_names, base_seed,
                                        nextAttemptIndex(h_index, trial.seed_index, target_trial_count, repeat_index),
